...
生成後は `docs/index.html` から各文書を開けます。Gitで管理するMarkdownを原本とし、ローカルに `docs/auth-implementation-plan.mdx` や `docs/spec-review.mdx` がある場合は併せてHTMLを生成します。MDXとHTMLはGit管理対象外です。回帰テスト、独立したJSON Schema検査、形式検証の手順は[検証ガイド](verification/README.md)にあります。
