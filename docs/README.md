...
Markdown の原本から HTML を生成するには、リポジトリのルートで実行する。

```sh
python3 scripts/render_docs.py
```

`docs/index.html`、`docs/yamai-protocol.html`、`docs/artifacts.html`、`verification/index.html`、`verification/quint/index.html` を生成する。生成には mdxr `0.2.0` を使用し、相互リンクも HTML の配置に合わせる。検査の実行方法と適合性の範囲は [検証ガイド](../verification/README.md) に従う。
