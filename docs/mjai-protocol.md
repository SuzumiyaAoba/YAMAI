...
本書中の `MUST`、`SHOULD` および `MAY` は、既存資料を引用する箇所を除き、[RFC 2119] の規範要件を表さない。YAMAI の規範要件は、組込み `riichi-4p` の採点規則を含め、[YRC 0003](yamai-protocol.md) `1.0-draft.9` が定義する。
...
本書は新しい MJAI 値を登録しない。YAMAI 用 registry は [YRC 0003] `1.0-draft.9` が別名前空間として定義する。
