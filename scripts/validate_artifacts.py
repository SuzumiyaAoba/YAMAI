...
PROTOCOL = "1.0-draft.8"
PROFILE = "riichi-4p"
PROFILE_REVISION = "1.0-draft.6"
...
PROFILE_HASH_INPUTS = [
    "schemas/yrc-0003/1.0-draft.8/profile/riichi-4p.schema.json",
    "schemas/yrc-0005/1.0-draft.6/riichi-4p-rules.schema.json",
    "schemas/yrc-0005/1.0-draft.6/scoring-vectors.schema.json",
    "registry/yrc-0003/1.0-draft.8/registry.json",
    "registry/yrc-0005/1.0-draft.6/registry.json",
    "test-vectors/yrc-0003/1.0-draft.8/vectors.json",
    "test-vectors/yrc-0005/1.0-draft.6/scoring.json",
]
...
def check_registry(schemas: SchemaSet) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    p = strict_load(ROOT / "registry/yrc-0003/1.0-draft.8/registry.json")
    r = strict_load(ROOT / "registry/yrc-0005/1.0-draft.6/registry.json")
    ...
    event_schema = schema_by_id(schemas, "urn:yamai:schema:yrc-0003:1.0-draft.8:event")
    ...
    action_schema = schema_by_id(schemas, "urn:yamai:schema:yrc-0003:1.0-draft.8:action")
    ...
    ack_schema = schema_by_id(schemas, "urn:yamai:schema:yrc-0003:1.0-draft.8:ack")
    ...
    expected_scoring_schema_files = {
        str(path.relative_to(ROOT))
        for path in (SCHEMA_ROOT / "yrc-0005" / "1.0-draft.6").glob("*.json")
    }
...
def profile_hash(protocol_registry: Dict[str, Any], rules_registry: Dict[str, Any]) -> str:
    manifest = strict_load(ROOT / "test-vectors/yrc-0003/1.0-draft.8/manifest.json")
    vectors = strict_load(ROOT / manifest["vectors"])
    scoring = strict_load(ROOT / manifest["scoring_vectors"])
    profile_schema = strict_load(ROOT / "schemas/yrc-0003/1.0-draft.8/profile/riichi-4p.schema.json")
    rules_schema = strict_load(ROOT / "schemas/yrc-0005/1.0-draft.6/riichi-4p-rules.schema.json")
    scoring_vectors_schema = strict_load(ROOT / "schemas/yrc-0005/1.0-draft.6/scoring-vectors.schema.json")
...
def check_manifest(schemas: SchemaSet, p: Dict[str, Any], r: Dict[str, Any]) -> Dict[str, Any]:
    path = ROOT / "test-vectors/yrc-0003/1.0-draft.8/manifest.json"
    manifest = strict_load(path)
    schema = schema_by_id(schemas, "urn:yamai:schema:yrc-0003:1.0-draft.8:vector-manifest")
    ...
    if manifest["schema_root"] != "schemas/yrc-0003/1.0-draft.8/message.schema.json":
...
def check_release_manifest(manifest: Dict[str, Any], p: Dict[str, Any], r: Dict[str, Any]) -> None:
    ...
    expected_vectors = [
        "test-vectors/yrc-0003/1.0-draft.8/manifest.json",
        manifest["vectors"],
    ...
    expected_schemas = {
        *p["schema_files"],
        "schemas/yrc-0003/1.0-draft.8/vector-manifest.schema.json",
        *r["schema_files"],
    }
...
def _check_scoring_fixture_semantics(fixture: Mapping[str, Any], base_rules: Mapping[str, Any], schemas: SchemaSet) -> None:
    ...
    schemas.validate(effective_rules, {"$ref": "urn:yamai:schema:yrc-0005:1.0-draft.6:riichi-4p-rules"})
    try:
...
def check_vectors(schemas: SchemaSet, manifest: Dict[str, Any]) -> int:
    vectors = strict_load(ROOT / manifest["vectors"])
    root_schema = schema_by_id(schemas, "urn:yamai:schema:yrc-0003:1.0-draft.8:message")
    checked = 0
...
def check_scoring(schemas: SchemaSet, rules_registry: Mapping[str, Any]) -> Tuple[int, int, int]:
    data = strict_load(ROOT / "test-vectors/yrc-0005/1.0-draft.6/scoring.json")
    scoring_schema_id = "urn:yamai:schema:yrc-0005:1.0-draft.6:scoring-vectors"
    ...
    for fixture in negatives:
        ...
        try:
            ...
            schemas.validate(effective, {"$ref": "urn:yamai:schema:yrc-0005:1.0-draft.6:riichi-4p-rules"})
    ...
    for summary in vector_items:
        ...
        schemas.validate(rules, {"$ref": "urn:yamai:schema:yrc-0005:1.0-draft.6:riichi-4p-rules"})
...
def main() -> int:
    try:
        ...
        schemas.check_refs()
        schemas.check_keyword_support()
        root = schema_by_id(schemas, "urn:yamai:schema:yrc-0003:1.0-draft.8:message")
        ...
        expected_ids = {"urn:yamai:schema:yrc-0003:1.0-draft.8:" + x for x in expected_kinds}
