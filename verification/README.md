...
| 14 | §7.5、§7.6.3–7.6.8 | V14/V23/V343–V350/V359–V360/V374–V383、採点fixture・N30–N36 | 全分解からの役・符・bonus・役満・支払い、槍槓の物理在庫、役IDの重複・排他・必要面子数と牌種条件・有効ルール・公開履歴、20符・25符の双方向条件 |
...
| 18 | §13.3 | V53–V62/V271/V351–V358/V367–V370、EventStateとsnapshot検査、test_session_contract | 未使用seq、手牌枚数、待ち要求、状態と残量、局間復元後の次局開始、連続prefixの確定状態・ID・元記録位置の保持、終局結果の不変性、原因eventの巻戻し・別payload・非event参照の拒否、途中観戦で未発行のevent番号を作らないこと |
...
| 30 | §3.1/6.4/8.1/12 | V63–V103/V108/V261/V273/V342、Receiver回帰 | Applyの原子性、errorの方向・優先順、同一seqの衝突と不正payloadの競合、同内容の過去eventへの原因参照 |
...
- game_contractは完全な1判断局面の候補生成、精算済み条件からの次局、受信者が観測できるevent状態を検査する。和了では役IDの重複・排他、門前／副露、適用ルール、公開されたリーチ・和了原因と20符・25符の例外条件との整合を検査してから、符・飜・役満による金額、本場・供託・責任払いを含む差額を照合する。他家の非公開手牌やホストの牌山順列は復元しない。非公開情報を要する役の判定には、ホストの完全情報と採点fixtureが必要である。
